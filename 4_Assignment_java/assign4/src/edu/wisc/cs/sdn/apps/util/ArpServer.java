...
public class ArpServer implements IFloodlightModule, IOFMessageListener
	...
	public boolean isCallbackOrderingPostreq(OFType type, String name)
		{
		return(OFType.PACKET_IN == type && name.equals(DeviceManagerImpl.MODULE_NAME));
		}
	...
	public net.floodlightcontroller.core.IListener.Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx)
		...
		log.info(String.format("Received ARP request for %s from %s", IPv4.fromIPv4Address(targetIP), MACAddress.valueOf(arp.getSenderHardwareAddress()).toString()));
		Iterator<? extends IDevice> deviceIterator = this.deviceProv.queryDevices(null, null, targetIP, null, null);
		...
		// Send the ARP reply
		log.info(String.format("Sending ARP reply %s->%s", IPv4.fromIPv4Address(targetIP), MACAddress.valueOf(deviceMac).toString()));
